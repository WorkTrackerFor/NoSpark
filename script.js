...
function generateQA(text) {
  ...
  lines.forEach(line => {
    if (line.includes(":") || line.includes("-") || line.includes("(")) {
      ...
      if (parts.length >= 2) qaPairs.push({ q: parts[0], a: parts.slice(1).join(" ") });
    }
  });

  // Fallback: pick first 10 lines as questions/answers
  if (qaPairs.length === 0) {
    lines.slice(0, 10).forEach(l => qaPairs.push({ q: l, a: "Answer: " + l }));
  }
...
$("#flashBtn").addEventListener("click", () => {
  ...
  container.innerHTML = qaPairs.map(pair => `
    <div class="qa-card">
...
$("#importBtn").addEventListener("click", () => {
  ...
  input.onchange = e => {
    ...
    reader.onload = ev => {
      ...
      if (data.flashcards) { $("#flashcards").innerHTML = data.flashcards; localStorage.setItem("flashcards", data.flashcards); }
      playSound("#sSuccess");
    };
